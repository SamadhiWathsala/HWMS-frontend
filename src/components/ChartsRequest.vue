  ...
  <v-row justify="start">
    <v-dialog v-model="dialog" persistent max-width="600px">
      <template v-slot:activator="{ on, attrs }">
        <v-btn color="primary" dark v-bind="attrs" v-on="on">Request to start mesurement chart</v-btn>
      </template>
      <v-card>
        <v-card-title>
          <span class="headline">Mesurement chart request form</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
              ...
              <v-col cols="12" sm="6">
                <v-select
                  v-model="chartType"
                  :items="['Blood Presure', 'Tempreture', 'Blood sugger', 'I/O chart', 'Pulse Rate']"
                  label="Mesurment Chart Type*"
                  required
                ></v-select>
              </v-col>
              <v-col cols="12" sm="6">
        ...
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" text @click="requestTreatment">Save</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-row>
